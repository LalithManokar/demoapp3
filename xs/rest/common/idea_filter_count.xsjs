...
var searchType = $.request.parameters.get("searchType") || 0;
...
var groupToken = $.request.parameters.get("cvt") || '';
var groupNameType = $.request.parameters.get("cvy") || 0;
var groupRoleType = $.request.parameters.get("cvr") || 0;
//	var oParameters = formatParams($.request.parameters);

try {
	t_filter_count = oHQ.procedure("SAP_INO", "sap.ino.db.idea::p_idea_filter_count").execute(searchType, searchToken, 
	    oCamaignId, phase, latestUpdate,
		status_code, resp_code, dueFrom, vote_count, vote_operator, 
		authorsToken, coachKeys, status_type, tagTokens, oIdeaFilterChange, dueTo,
		ifcode, ifop, ifvalue, iscode, isop, isvalue, itcode, itop, itvalue, ideaFormId, 
		tagTokens1, tagTokens2, tagTokens3, tagTokens4,
		groupToken, groupNameType, groupRoleType, 
		bShowBackoffice);
	var out_put = {};
