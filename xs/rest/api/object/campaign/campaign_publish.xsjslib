...
function campaign_publish(user, return_inf) {
	var errorMessages;
	try {
	...
	var oResponse;
	try {
		oResponse = Campaign.submit(user.Parameters.Campaign_ID);
	} catch (e) {
	...
	return_inf = util.get_inf(user, return_inf); //get successful information
