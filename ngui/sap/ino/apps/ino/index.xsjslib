function getApplication() {
    ...
    var sSql = `SELECT TOP 4 CODE, VALUE FROM "sap.ino.db.basis.ext::v_ext_system_setting"  
    WHERE (CODE = 'sap.ino.config.OPEN_GROUP_FOR_COMMUNITY_USER' AND VALUE = '1') 
    OR (CODE = 'sap.ino.config.DEFAULT_SYSTEM_CDN_ACTIVE' AND VALUE = '1') 
    OR (CODE='sap.ino.config.DISABLE_GOOGLE_TRANSLATION_TIP' AND VALUE = '1')
    OR (CODE='sap.ino.config.DEFAULT_CDN_LINK_UI5_LIB_VERSION')
    `;
    var oSystemSetting = oHQ.statement(sSql).execute();
    var oLanguage = oHQ.statement('SELECT LOCALE FROM "sap.ino.db.iam.ext::v_ext_logon_user"').execute();
    
    var bDevelopment = $.request.parameters.get("env") === "development";
    var register = $.import('sap.ino.setup.xslib','register').aRegister;
    var lasted = register[register.length - 1];
    var version = lasted.version + '.' + lasted.version_sp + '.' + lasted.version_patch;
    ...
    var sContent = "<!DOCTYPE HTML>" +
        "<html>" +
            ...
            if(oSystemSetting && oSystemSetting.length > 0 && _.findWhere(oSystemSetting,{"CODE":"sap.ino.config.DISABLE_GOOGLE_TRANSLATION_TIP"})){
            ...
            "<script id=\"sap-ui-bootstrap\" src=\"" ;
                if(oSystemSetting && oSystemSetting.length > 0 && _.findWhere(oSystemSetting,{"CODE":"sap.ino.config.DEFAULT_SYSTEM_CDN_ACTIVE"})){
                ...
                if($.request.parameters.get("sap-ui-debug") === "true"){
                    sContent = sContent.replace("sap-ui-core.js", "sap-ui-core-dbg.js")
                }
                //sContent += "https://sapui5.hana.ondemand.com/1.71.47/resources/sap-ui-core.js";
                var sLan = "en";
                if(oLanguage && oLanguage.length > 0 && oLanguage[0].LOCALE){
                    sLan = oLanguage[0].LOCALE;
                }
                sContent += "\" data-sap-ui-language=\"" + sLan + "\" ";
                sContent += " data-sap-ui-libs=\"sap.m,sap.ui.unified,sap.ui.layout,sap.uxap,sap.tnt";
                if(oSystemSetting && oSystemSetting.length > 0 && _.findWhere(oSystemSetting,{"CODE":"sap.ino.config.OPEN_GROUP_FOR_COMMUNITY_USER"})){
                    sContent += ",sap.ui.table\" ";
                }else{
                    sContent += "\" ";
                }
                sContent +=  "data-sap-ui-theme=\"sap_bluecrystal\" " +
                "data-sap-ui-xx-bindingSyntax=\"complex\" " +
                "data-sap-ui-async=\"true\" "  +
                "data-sap-ui-compatVersion=\"edge\" " +
                "data-sap-ui-resourceroots='{\"sap.ino\" : \"/sap/ino/ngui/sap/ino/\"}' " + 
                "data-sap-ui-frameOptions=\"deny\" " +
                "data-sap-ui-xx-libraryPreloadFiles=\"sap.ino.corelib:json\"" + 
                "data-sap-ui-versionedLibCss=\"true\">" +
            "</script>" + 
            "<script type=\"text/javascript\">" +
                "var gSAPInoAppName = \"sap.ino.ngui.sap.ino.apps.ino\";" +
            "</script>";
    if(bDevelopment) {
        sContent += 
				"<!-- resourceroots is overwritten in the component -->" +
				"<script src=\"/sap/ino/ngui/sap/ino/apps/ino/bootstrap.js\"></script>";
	} 
	sContent += 
	        "<script type=\"text/javascript\">" +
                ...
                if(!bDevelopment) {
                    ...
                    "var oView = sap.ui.xmlview({" +
                        "id : \"sap-ino-main\"," +
                        "viewName : \"sap.ino.vc.shell.InoShell\"" +
                    "});" +
