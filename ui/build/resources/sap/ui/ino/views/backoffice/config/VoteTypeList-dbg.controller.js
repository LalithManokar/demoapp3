...
sap.ui.core.mvc.Controller.extend("sap.ui.ino.views.backoffice.config.VoteTypeList", jQuery.extend({},
        sap.ui.ino.views.common.MasterDetailController, {
            getSettings : function() {
                var mSettings = {
                    ...
                    mTableViews : {
                        "staging" : {
                            "default" : true,
                            sBindingPathTemplate : "/StagingVoteTypeSearchParams(searchToken='{searchTerm}')/Results", 
                    ...
                    sApplicationObject : "sap.ui.ino.models.object.VoteTypeStage"
                };
            ...
            onNavigateToModel : function(iId, bEdit) {
                ...
                var oModifyView = sap.ui.jsview("sap.ui.ino.views.backoffice.config.VoteTypeModify");
                var sMode = "display";
                if(bEdit === true){
                    sMode = "edit";
                }
            ...
            onCopyAsPressed : function(){
                ...
                if (oBindingContext) {
                    ...
                    var sPrefix = oBundle.getText("BO_VOTE_TYPE_COPY_CODE_PREFIX");
            ...
            onCopyPressed : function(sCopyCode) {
                ...
                var oCopyRequest = sap.ui.ino.models.object.VoteTypeStage.copy(iId, {
                    ID : -1,
                ...
                oCopyRequest.done(function(oResponse) {
                    ...
                    setTimeout(function() {
                        ...
                        var oMessageParameters = {
                            key : "MSG_VOTE_TYPE_COPIED",
                            level : sap.ui.core.MessageType.Success,
                            parameters : [],
                            group : "configuration_vote",
                            text : oMsg.getResourceBundle().getText("MSG_VOTE_TYPE_COPIED")
                        };
            ...
            onDeletePressed : function() {
                sap.ui.ino.application.backoffice.Application.getInstance().removeNotificationMessages("configuration_vote");
                ...
                function deleteInstance(bResult) {
                    if (bResult) {
                        var oDeleteRequest = sap.ui.ino.models.object.VoteTypeStage.del(iId);
                        that.executeActionRequest(oDeleteRequest, that._oDeleteButton, false);
                        oDeleteRequest.done(function() {                            
                            var oMsg = sap.ui.getCore().getModel(sap.ui.ino.application.ApplicationBase.MODEL_MSG);
                            var oMessageParameters = {
                                key : "MSG_VOTE_TYPE_DELETED",
                                level : sap.ui.core.MessageType.Success,
                                parameters : [],
                                group : "configuration_vote",
                                text : oMsg.getResourceBundle().getText("MSG_VOTE_TYPE_DELETED")
                            };
                ...
                var oBundle = sap.ui.getCore().getModel(sap.ui.ino.application.ApplicationBase.MODEL_TEXT).getResourceBundle();
                sap.ui.ino.controls.MessageBox.confirm(oBundle.getText("BO_VOTE_TYPE_INS_DEL"),
                        deleteInstance, oBundle.getText("BO_VOTE_TYPE_TIT_DEL"));                
            },
            ...
            updatePropertyModel : function() {
                ...
                if (oSelectedRowContext) {
                    ...
                    oPropertyModel = new sap.ui.ino.models.core.PropertyModel(
                            "sap.ino.xs.object.campaign.VoteTypeStage", iModelId, {
                            	staticActions : [{"create": {}}],
                                actions : ["create", "update", "copy", "del"]
                            });                    
                } else {
                	oPropertyModel = new sap.ui.ino.models.core.PropertyModel(
                            "sap.ino.xs.object.campaign.VoteTypeStage", 0, {
                            	staticActions : [{"create": {}}],
                                actions : ["create", "update", "copy", "del"]
                    });                    
                } 
            ...
            _handleCopyFail : function(oResponse){
            	var oView = this.getView();
                var aActionMessages = sap.ui.ino.models.core.MessageSupport.convertBackendMessages(oResponse.MESSAGES, oView, "configuration_vote");
                var oApp = sap.ui.ino.application.backoffice.Application.getInstance();
                if(aActionMessages){
                	...
                	var oMessageParameters = {
                            key : "MSG_VOTE_TYPE_COPY_FAILURE",
                            level : sap.ui.core.MessageType.Error,
                            parameters : [],
                            group : "configuration_vote",
                            text : "MSG_VOTE_TYPE_COPY_FAILURE"
                    };
                    var oMessage = new sap.ui.ino.application.Message(oMessageParameters);
                    oApp.addNotificationMessage(oMessage);
                    oView.getCopyAsDialog().close();
