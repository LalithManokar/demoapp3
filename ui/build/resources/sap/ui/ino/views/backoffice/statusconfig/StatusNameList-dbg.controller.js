...
sap.ui.core.mvc.Controller.extend("sap.ui.ino.views.backoffice.statusconfig.StatusNameList", jQuery.extend({},
	sap.ui.ino.views.common.MasterDetailController, {
		getSettings: function() {
			var mSettings = {
				...
				mTableViews: {
					"staging": {
						"default": true,
						sBindingPathTemplate: "/SearchStagingStatusNameParams(searchToken='{searchTerm}')/Results",
						oSorter: new sap.ui.model.Sorter("CODE", false),
						aVisibleActionButtons: ["BUT_CREATE", "BUT_COPY", "BUT_EDIT", "BUT_DELETE"],
						aVisibleColumns: ["PACKAGE_ID", "CODE", "DEFAULT_TEXT", "STATUS_TYPE", "CHANGED_AT", "CHANGED_BY"]
					}
				},
				sApplicationObject: "sap.ui.ino.models.object.StatusNameStage"
			};
		...
		onNavigateToModel: function(iId, bEdit) {
			...
			var oModifyView = sap.ui.jsview("sap.ui.ino.views.backoffice.statusconfig.StatusNameModify");
			var sMode = "display";
			if (bEdit === true) {
				sMode = "edit";
			}
		...
		onCopyPressed: function() {
			...
			if (oBindingContext) {
				...
				var sPrefix = oBundle.getText("BO_STATUS_NAME_LIST_COPY_CODE_PREFIX");
		...
		onPressCopy: function(sCopyCode) {
			...
			var oCopyRequest = sap.ui.ino.models.object.StatusNameStage.copy(iId, {
				ID: -1,
			...
			oCopyRequest.done(function(oResponse) {
				...
				setTimeout(function() {
					...
					var oMessageParameters = {
						key: "MSG_STATUS_NAME_ADMINISTRATION_COPIED",
						level: sap.ui.core.MessageType.Success,
						parameters: [],
						group: "configuration_status_name",
						text: oMsg.getResourceBundle().getText("MSG_STATUS_NAME_ADMINISTRATION_COPIED")
					};
		...
		updatePropertyModel: function() {
			...
			if (oSelectedRowContext) {
				...
				oPropertyModel = new sap.ui.ino.models.core.PropertyModel(
					"sap.ino.xs.object.status.StatusStage", iModelId, {
						staticActions: [{
							"create": {}
						}],
				...
				oPropertyModel = new sap.ui.ino.models.core.PropertyModel(
					"sap.ino.xs.object.status.StatusStage", 0, {
						staticActions: [{
							"create": {}
						}],
						actions: ["create", "update", "copy", "del"]
		...
		onDeletePressed: function() {
			sap.ui.ino.application.backoffice.Application.getInstance().removeNotificationMessages("configuration_status_name");
			...
			function deleteInstance(bResult) {
				if (bResult) {
					var oDeleteRequest = sap.ui.ino.models.object.StatusNameStage.del(iId);
					that.executeActionRequest(oDeleteRequest, that._oDeleteButton, false);
					oDeleteRequest.done(function() {
						var oMsg = sap.ui.getCore().getModel(sap.ui.ino.application.ApplicationBase.MODEL_MSG);
						var oMessageParameters = {
							key: "MSG_STATUS_NAME_DELETED",
							level: sap.ui.core.MessageType.Success,
							parameters: [],
							group: "configuration_status_name",
							text: oMsg.getResourceBundle().getText("MSG_STATUS_NAME_DELETED")
						};
			...
			var oBundle = sap.ui.getCore().getModel(sap.ui.ino.application.ApplicationBase.MODEL_TEXT).getResourceBundle();
			sap.ui.ino.controls.MessageBox.confirm(oBundle.getText("BO_STATUS_NAME_INS_DEL"),
				deleteInstance, oBundle.getText("BO_STATUS_NAME_TIT_DEL"));
		},
		_handleCopyFail: function(oResponse) {
			...
			if (aActionMessages) {
				...
				var oMessageParameters = {
					key: "MSG_STATUS_NAME_COPY_FAILURE",
					level: sap.ui.core.MessageType.Error,
					parameters: [],
					group: "configuration_status_name",
					text: "MSG_STATUS_NAME_COPY_FAILURE"
				};
				var oMessage = new sap.ui.ino.application.Message(oMessageParameters);
				oApp.addNotificationMessage(oMessage);
				oView.getCopyAsDialog().close();
