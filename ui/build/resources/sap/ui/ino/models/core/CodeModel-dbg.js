 ...
 */
jQuery.sap.declare("sap.ui.ino.models.core.CodeModel");
...
(function() {
	...
	var ObjectType = sap.ui.ino.models.core.ApplicationObject.ObjectType;
	...
	sap.ui.ino.models.core.CodeModel = new sap.ui.model.json.JSONModel({});

	var oThis = sap.ui.ino.models.core.CodeModel;

	oThis.sDefaultBindingMode = sap.ui.model.BindingMode.OneWay;
	oThis.mSupportedBindingModes = {
		"OneWay": true,
		"OneTime": true,
		"TwoWay": false
	};
	// clean up code buffer, so that new / deleted entries are reflected
	sap.ui.ino.models.core.ApplicationObjectChange.attachChange(undefined, function(oEvent) {
		...
		if (oMetadata.type === ObjectType.Stage) {
			...
			sap.ui.ino.models.core.CodeModel.refresh(sConfigObjectName);
		}else if(oMetadata.name === "sap.ino.xs.object.subresponsibility.ResponsibilityStage"){
			sap.ui.ino.models.core.CodeModel.refresh(oMetadata.name);
		}
	});
	...
	function _getODataModel() {
		if (_oODataModel === null) {
			_oODataModel = new sap.ui.model.odata.ODataModel(Configuration.getFullApplicationPath("sap.ino.config.URL_PATH_OD_CONFIGURATION"), true);
		}
		...
		oODataModel.read(sPath, null, null, false, function(oResponse) {
			if (oResponse && oResponse.results && oResponse.results.length > 0) {
				jQuery.each(oResponse.results, function(iIndex, oCode) {
					...
					if (oCode.DEFAULT_TEXT && oCode.TEXT === null) {
	...
	function _getCode(sConfigObjectNode, vCodeFilter, mParameters) {
		var aCodes = sap.ui.ino.models.core.CodeModel.getProperty("/" + sConfigObjectNode);
		var aParamCodes = undefined;
		if (mParameters) {
			aParamCodes = sap.ui.ino.models.core.CodeModel.getProperty("/" + JSON.stringify(mParameters) + "/" + sConfigObjectNode);
		}
		if (aCodes === null || aCodes === undefined) {
			aCodes = _readCodeValues(sConfigObjectNode);
			sap.ui.ino.models.core.CodeModel.setProperty("/" + sConfigObjectNode, aCodes);
		}
		...
		if (mParameters && !aParamCodes) {
			...
			aCodes = aParamCodes;
			var oCodeTable = sap.ui.ino.models.core.CodeModel.getProperty("/" + JSON.stringify(mParameters)) || {};
			oCodeTable[sConfigObjectNode] = aParamCodes;
			sap.ui.ino.models.core.CodeModel.setProperty("/" + JSON.stringify(mParameters), oCodeTable);
		}
	...
	function _getResourceModel(sConfigObjectNode) {
		...
		var oMetadata = sap.ui.ino.models.core.MetaModel.getApplicationObjectMetadata(sObjectName);
		if (!oMetadata) {
	...
	sap.ui.ino.models.core.CodeModel.bindList = function(sPath, oContext, aSorters, aFilters, mParameters) {
		...
		if (!oContext && sPath != "/") {
			...
			sap.ui.ino.models.core.CodeModel.getCodes(sCodeTableName, undefined, mParameters);
		}
	 ...
	 */
	sap.ui.ino.models.core.CodeModel.getCodes = function(sConfigObjectNode, fnFilterFunction, mParameters) {
	...
	sap.ui.ino.models.core.CodeModel.getText = function(sConfigObjectNode, sCode) {
	 ...
	 */
	sap.ui.ino.models.core.CodeModel.getLongText = function(sConfigObjectNode, sCode) {
	 ...
	 */
	sap.ui.ino.models.core.CodeModel.getFormatter = function(sConfigObjectNode) {
		return _getTextFormatter(sap.ui.ino.models.core.CodeModel.getText, sConfigObjectNode);
	};
	 ...
	 */
	sap.ui.ino.models.core.CodeModel.getLongTextFormatter = function(sConfigObjectNode) {
		return _getTextFormatter(sap.ui.ino.models.core.CodeModel.getLongText, sConfigObjectNode);
	};
	 ...
	 */
	sap.ui.ino.models.core.CodeModel.getConfigObjectNodeForValueOptionList = function(sValueOptionListCode) {
		return VALUE_OPTION_LIST_PREFIX + sValueOptionListCode;
	};

	sap.ui.ino.models.core.CodeModel.refresh = function(sObjectName) {
		if (!sObjectName) {
			sap.ui.ino.models.core.CodeModel.setData({});
			return;
		}
		var aPathsToInitialize = [];
		jQuery.each(sap.ui.ino.models.core.CodeModel.getData(), function(sProperty, vValue) {
		...
		jQuery.each(aPathsToInitialize, function(iIndex, sPath) {
			sap.ui.ino.models.core.CodeModel.setProperty(sPath, null);
		});
	};
