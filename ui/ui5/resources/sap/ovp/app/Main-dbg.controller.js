sap.ui.define(["sap/ui/core/mvc/Controller", "sap/m/library", "sap/ui/model/odata/ODataUtils", "sap/ovp/cards/generic/Component",
        ...
        "sap/base/Log", "sap/base/util/merge", "sap/base/security/encodeURL", "sap/base/util/ObjectPath", "sap/ui/thirdparty/jquery"],
    ...
    function (Controller, SapMLibrary, ODataUtils, Component, NavigationHandler, MessageBox, CommonUtils, PersonalizationUtils,
        ...
        TemplateBaseExtension, ControllerExtension, OVPUtils, Log, merge, encodeURL, ObjectPath, jQuery) {
        ...
        return Controller.extend("sap.ovp.app.Main", {
            ...
            verifyGlobalFilterLoaded: function () {
                ...
                var oFilterNotFulfilledPage = this.getView().byId("ovpFilterNotFulfilledPage");
                if (oFilterNotFulfilledPage) {
                    oFilterNotFulfilledPage.setVisible(true);
                }
		...
		//triggering a async call so that card components should be created
                ...
                ControlPersonalizationAPI.resetChanges(aCardForDeletion,aChangeTypeForDeletion).finally(function(){
                    ControlPersonalizationAPI.addPersonalizationChanges({
                        ...
                        ControlPersonalizationAPI.saveChanges(aChanges, layout).then(function () {
                            ...
                            Log.error("Personalization changes are not being saved by 'saveChanges' function");
                        });
                    }, function () {
                        Log.error("Personalization changes are not being added by 'addPersonalizationChanges' function");
                    });
                });
            },
            /**
            ...
            _onManageCardsMenuButtonPress: function() {
                ...
                if (sLayoutName === "sap.ovp.ui.DashboardLayout") {
                    this._getCardArrayAsVariantFormatDashboard().done(function (aRes) {
                        oManageCardsDialogModel = jQuery.extend(true, [], aRes);
                    ...
                    oManageCardsDialogModel = jQuery.extend(true, [], this.aOrderedCards);
                    var oModel = new JSONModel({
                        cards: oManageCardsDialogModel
                    });
                    this.manageCardsDialog.setModel(oModel);
            ...
            _getCurrentAppState: function (variantManagementSaved) {
                ...
                if (!variantManagementSaved) {
                 ...
                 if (typeof event === "string" && event !== 'variantmanagementSave') {
                    //Get data from custom filters
             ...
             * @private
             */
            _storeCurrentAppStateAndAdjustURL: function (event, variantManagementSaved) {
                ...
                if (event && !event.oSource._bDirtyViaDialog) {
                    return;
                }
