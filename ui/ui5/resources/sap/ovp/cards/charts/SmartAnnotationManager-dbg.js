 ...
 */
sap.ui.define(["sap/ui/thirdparty/jquery", "sap/ovp/cards/charts/Utils", "sap/ui/model/odata/CountMode", "sap/ovp/cards/charts/VizAnnotationManager",
            "sap/ovp/cards/AnnotationHelper", "sap/ui/core/format/NumberFormat", "sap/ui/core/format/DateFormat", "sap/viz/ui5/api/env/Format",
            "sap/viz/ui5/controls/VizTooltip", "sap/ovp/app/resources", "sap/base/Log", "sap/base/util/each", "sap/base/util/merge"],
    function (jQuery, Utils, CountMode, VizAnnotationManager, CardAnnotationHelper, NumberFormat, DateFormat, VizFormat, VizTooltip, OvpResources, Log, each, merge) {
        "use strict";
...
//             if (maxItems) {
//                 if (maxItems == "0") {
//                     Log.error("OVP-AC: Analytic card Error: maxItems is configured as " +
//                     maxItems);
//                     ret += "}";
//                     return ret;
//                 }
//                 if (!/^\d+$/.test(maxItems)) {
//                     Log.error("OVP-AC: Analytic card Error: maxItems is Invalid. " +
...
//             if (bFilter) {
//                 each(oSelectionVariant.SelectOptions, function () {
//                     var prop = this.PropertyName.PropertyPath;
//                     each(this.Ranges, function () {
...
//										showScale: false,
        ...
        function checkExists(term, annotation, type, bMandatory, logViewId, contentFragment) {
            ...
            if (contentFragment &&
                ...
                Log.error(logViewId + errorMessages.CARD_ERROR + errorMessages.MISSING_CHARTTYPE +
        ...
        function validateCardConfiguration(oController) {
            ...
            if (!(oCardsModel = oController.getCardPropertiesModel())) {
                Log.error(logViewId + errorMessages.CARD_ERROR + "in " + errorMessages.CARD_CONFIG +
...
//							formatString: bDonutChart ? '0.0%' : vizProperties.plotArea.dataLabel.formatString
            ...
            [config.dimensions, config.measures].forEach(function (entry, i) {
                ...
                if (entry.min && oProperty.length < entry.min) {
                    Log.error(errorMessages.CARD_ERROR + "in " + chartType +
