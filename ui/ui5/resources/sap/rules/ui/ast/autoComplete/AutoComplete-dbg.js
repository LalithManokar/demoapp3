sap.ui.define(["sap/rules/ui/ast/autoComplete/dataStructure/Stack",
	...
	function (Stack, TermsProvider, OperatorProvider, FunctionProvider, astBundleLibrary, Constants, TermNode, ComparisionOperatorStack,
		...
		AutoComplete.prototype._constructFunctionsAutocomplete = function (oAutoCompleteJson, oTopNode, oTopStack, bIsResultContext,
			...
			if (lastFunctionalStack) {
				...
				if (oTopStack.getSize() != 0) {
					...
					var arg = argumentsMetadata[lastFunctionalStackSize - 1];
					// First Argument
					if (lastFunctionalStackSize == 0 || !("referenceIndex" in arg)) {

						if ("businessDataTypeList" in arg) {
							aBusienssDataTypeList = arg.dataObjectTypeList;
							...
							for (var lBusinessDataListIterator = 0; lBusinessDataListIterator < aBusienssDataTypeList.length; lBusinessDataListIterator++) {
								for (var lDataObjectListIterator = 0; lDataObjectListIterator < aDataObjectTypeList.length; lDataObjectListIterator++) {
									this._categorizeFunctionsByCatgory_BusinessDataType_DataObjectType(oCategories, sFunctionCategory,
										aBusienssDataTypeList[lBusinessDataListIterator], aDataObjectTypeList[lDataObjectListIterator], bIsResultContext);
								}
							}
						} else {
							// No businessDatatype found, get all the functions with matching category and dataobject type
							aDataObjectTypeList = arg.businessDataTypeList;
							for (var lOuterIndex = 0; lOuterIndex < aDataObjectTypeList.length; lOuterIndex++) {
								aAllFunctions = FunctionProvider.getInstance().
								getAllFunctionsGivenCategoryAndDataObjectType(sCategory, aDataObjectTypeList[lOuterIndex]);
						...
						var referenceArgumentNode = this._getNodeRecursively(lastFunctionalStack.peek(arg.referenceIndex));
		...
		AutoComplete.prototype._categorizeFunctionsByCatgory_BusinessDataType_DataObjectType = function (oCategories, sCategory,
			sBusienssDataType, sDataObjectType, bIsResultContext) {
			var aAllFunctions = FunctionProvider.getInstance().getAllFunctionsGivenCategoryAndDataObjectAndBusinessDataType(sCategory,
				sDataObjectType, sBusienssDataType);
