sap.ui.define([
	...
	"sap/ui/model/FilterProcessor"
		...
		MessageUtils, KeyCodes, merge, FilterProcessor) {
	"use strict";
	...
	var oMethods = {
		getMethods: function(oViewProxy, oTemplateUtils, oController) {
			...
			// Save of draft and non-draft case. Forwards either to onActivateImpl (draft) or onSaveImpl (non-draft)
				...
				if (fnIsSmartTableWithInlineCreate(oEvent.getSource())) {
					...
					var fnGroupExtended = fnGroup && function(oContext) {
						...
						if (oObject.IsActiveEntity || oObject.HasActiveEntity) {
							var oRet = Object.assign({}, fnGroup(oContext));
			...
			function onDeleteImpl() {
				...
				oBeforeDeleteExtensionPromise.then(function(oExtensionResult) {
					Object.assign(oDialogParameter, oExtensionResult);
			...
			*/
			...
			function fnDeleteEntries(oEvent) {
				...
				if (aContexts.length > 1) {
					...
					var sItemText = oTemplateUtils.oCommonEventHandlers.getSelectedItemContextForDeleteMessage(oSmartTable, aContexts[0]);
					oDialogParameter.title = oTemplateUtils.oCommonUtils.getText("ST_GENERIC_DELETE_TITLE");
					oDialogParameter.text = oTemplateUtils.oCommonUtils.getContextText("DELETE_SELECTED_ITEM", oSmartTable.getId(), null, sItemText);
				}
			...
			function fnHandleVisibilityChangeOfSubsection(bIsGettingVisible, oSubSectionInfoObject, oSubSection) {
				...
				bIsGettingVisible = bIsGettingVisible || fnCheckInitiallyVisibleSubSection(oSubSection);
			...
			// Check & returns true in case Sub Object should be visible initially
			// As per the logic the first three sub sections are set as visible initially
			function fnCheckInitiallyVisibleSubSection(oSubSection) {
				var sSubSectionId = oSubSection.getId();
				return aActiveSubSectionId.some(function(oCurrentSubSectionId) {
					if (oCurrentSubSectionId === sSubSectionId) {
						Log.info("Initially Visible Subsection: " + sSubSectionId , "sap.suite.ui.generic.template.ObjectPage.controller.ControllerImplementation");
						return true;
					}
			...
			oControllerImplementation.handlers = Object.assign(oBase.handlers, oControllerImplementation.handlers);
