sap.ui.define([
		...
		"sap/suite/ui/generic/template/js/StableIdHelper"
			...
			OverflowToolbar, OverflowToolbarLayoutData, CustomData, Device, SapMLibrary, SapCoreLibrary, FilterOperator, DatePicker, Log, InvisibleText, deepEqual, StableIdHelper) {
	"use strict";
	...
	var vfdController = Controller.extend("sap.suite.ui.generic.template.AnalyticalListPage.controller.VisualFilterDialogController", {
				...
				if (!Array.isArray(this.filterCompList[i])) {
					...
					if (this.filterCompList[i].obj.shownInFilterDialog) {
						...
						var oCustomListItem = new CustomListItem({
							id: StableIdHelper.getStableId({type: "VisualFilter", subType: "CustomListItem", sContainerType:"VisualFilterDialog", sParentProperty: that.filterChartList[i].getParentProperty()}),
							content: oChartBox
						});
		...
		*/
		...
		_addChartCustomToolbar: function(obj, idx) {
			...
			sId = StableIdHelper.getStableId({type: "VisualFilter", subType: "F4Button", sContainerType:"VisualFilterDialog", sParentProperty: props.parentProperty}),
			count,
			items = [
					new Button({
						type: "Transparent",
