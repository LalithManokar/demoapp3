	...
	function(BaseObject, MessageToast, ModelUtil, ActionUtil, MessageUtils, MessageBox, CRUDHelper, testableHelper) {
			...
			function saveEntityImpl(resolve, reject) {
				...
				oServices.oTransactionController.triggerSubmitChanges().then(function(oResponse) {
					...
					if (oContextFilter){
						...
						var oMessageBinding = oMessageModel.bindList("/", null, null, [oContextFilter]);
						var aMessages = oMessageBinding.getContexts().map(function(oContext){
		...
		return BaseObject.extend("sap.suite.ui.generic.template.lib.CRUDManager", {
				constructor: function(oController, oComponentUtils, oServices, oCommonUtils, oBusyHelper) {
					Object.assign(this, getMethods(oController, oComponentUtils, oServices, oCommonUtils, oBusyHelper));
				}
			});
	});
