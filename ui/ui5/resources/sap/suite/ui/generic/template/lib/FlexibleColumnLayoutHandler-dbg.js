	...
	function(BaseObject, FlexibleColumnLayoutSemanticHelper, fioriLibrary) {
		...
		function getMethods(oFlexibleColumnLayout, oNavigationControllerProxy) {
			...
			function fnHandleBeforeRouteMatched(oCurrentIdentity){
				...
				if (oCurrentIdentity.treeNode.fCLLevel === 1 || oCurrentIdentity.treeNode.fCLLevel === 2){
					var oNewUiState = oFlexibleColumnLayoutSemanticHelper.getCurrentUIState();
					// During startup FlexibleColumnLayoutSemanticHelper might respond with an empty visibility state
					var bNewUiStateReliable = oNewUiState.beginColumn || oNewUiState.midColumn || oNewUiState.endColumn;
					if (bNewUiStateReliable ? oNewUiState.columnsVisibility.midColumn : !isFullscreenLayout(sCurrentLayout)){
						var iDownto = 1 - (oNewUiState.columnsVisibility.beginColumn || !bNewUiStateReliable);
			...
			// before the draft was created. This identity is given by oIdentityBefore. However, the sppstates might be different from this state.
		...
		return BaseObject.extend("sap.suite.ui.generic.template.lib.FlexibleColumnLayoutHandler", {
			constructor: function(oFlexibleColumnLayout, oNavigationControllerProxy) {
				Object.assign(this, getMethods(oFlexibleColumnLayout, oNavigationControllerProxy));
			}
		});
	});
