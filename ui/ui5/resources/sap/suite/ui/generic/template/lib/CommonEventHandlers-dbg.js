sap.ui.define(["sap/ui/base/Object",
	...
	function getMethods(oController, oComponentUtils, oServices, oCommonUtils) {
				...
				Promise.all(aPromise).then(function(aResponses) {
					for (var i = 0; i < selectedItems.length; i++) {
						...
						if (!oEntity.IsActiveEntity) { // if the entity is not an active entity, we can assume it is a draft
							...
							// check if first and last name are provided. If not then take technical username
					...
					if (mJSONData.itemsCount > 1) {
						...
						if (mJSONData.lockedItemsCount > 0) {
							...
							var sItemText = fnGetSelectedItemContextForDeleteMessage(oSmartTable, selectedItems[0]); //To redefine the message in the delete dialog from Delete "Object?" to "Delete Object 500000124?"
							mJSONData.text.text = oCommonUtils.getText("ST_GENERIC_DELETE_SELECTED", sItemText);
						}
					}
		...
		/*
			...
			* @return String - Value of the property that is mapped to the Title property of the UI.HeaderInfo
		*/
		function fnGetSelectedItemContextForDeleteMessage(oSmartTable, oSelectedItem) {
			var aTableCustomData = oSmartTable.getCustomData();
			var sHeaderTitleAnnotationPath;
			var sContextPath;
			var oModel = oSelectedItem.getModel();
			var oData = oModel.oData;
			var sContextText;
			for (var index in aTableCustomData) {
				if (aTableCustomData[index].getKey() && aTableCustomData[index].getKey() === "headerTitleString" && aTableCustomData[index].getValue()) {
					//If a string mapped to the title, return it directly.
					sContextText =  aTableCustomData[index].getValue() || "";
					break;
				}
				if (aTableCustomData[index].getKey() && aTableCustomData[index].getKey() === "headerTitlePath" && aTableCustomData[index].getValue()) {
					//If a path is mapped to the title, return the value contained in that path.
					sHeaderTitleAnnotationPath =  aTableCustomData[index].getValue();
					sContextPath = oSelectedItem.getPath().split("/")[1];
					sContextText =  (oData[sContextPath] && oData[sContextPath][sHeaderTitleAnnotationPath]) || "";
					break;
				}
			}
			return sContextText;
		...
		function fnBuildSelectionVariantForNavigation(oOutbound, oLineContext, oPageContext, sFilterBarSelectionVariant){
			...
			var oMixedContextObject = Object.assign({}, oOutboundParametersEmpty, oPageContextObject, oLineContextObject, oOutboundParameters);
			...
			if (sOutboundTarget !== undefined && sOutboundTarget !== null && oXApplNavigation) {
				...
				if (oCrossApp) {
					...
					if (!oTablePathProp) {
						...
						var oNavParams = Object.assign({}, oParam, oOutboundParameters);
						var oNavArguments = {
								target : oTarget,
								params : oNavParams
						};
...
//			}
			...
			if (aSelectedItems && aSelectedItems.length > 0) {
				...
				oBeforeDeleteExtensionPromise.then(function(oExtensionResult){
					...
					oDialogParameterPromise.then(function(oDialogParameter){
						Object.assign(oDialogParameter.text, oExtensionResult);
	...
	return BaseObject.extend("sap.suite.ui.generic.template.lib.CommonEventHandlers", {
		constructor: function(oController, oComponentUtils, oServices, oCommonUtils) {
			Object.assign(this, getMethods(oController, oComponentUtils, oServices, oCommonUtils));
		}
	});
});
