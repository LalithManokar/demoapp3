sap.ui.define([
	...
	"sap/suite/ui/generic/template/js/StableIdHelper"
		...
		CustomData, Text, OverflowToolbarLayoutData, FilterOperator, Device, SimpleType, SapCoreLibrary, SapMLibrary, DatePicker, Log, InvisibleText, StableIdHelper) {
	"use strict";
	...
	*/
	...
	SmartVisualFilterBar.prototype._createHeaderItems = function (sPath, sType, oProperties) {
		...
		sId = this.getView().sId + "--" + StableIdHelper.getStableId({type: "VisualFilter", subType: "VBox", sContainerType:"VisualFilterBar", sParentProperty: oFilterItem.getParentProperty()});
		var oCell = new VBox({
			id: sId,
	...
	*/
	...
	SmartVisualFilterBar.prototype._createTitleToolbar = function(props, filterItem) {
		...
		if (oInput) {
			...
			sId = this.getView().sId + "--" + StableIdHelper.getStableId({type: "VisualFilter", subType: "F4Button", sContainerType:"VisualFilterBar", sParentProperty: filterItem.getParentProperty()});
	...
	SmartVisualFilterBar.prototype._createFilterItemOfType = function(type, properties, bIsBar) {
		var compInst;
		var sId = StableIdHelper.getStableId({type: "VisualFilter", subType: "FilterItemMicroChart", sContainerType: (bIsBar ? "VisualFilterBar" : "VisualFilterDialog"), sParentProperty: properties.parentProperty});
		sId = this.getView().sId + "--" + sId;
