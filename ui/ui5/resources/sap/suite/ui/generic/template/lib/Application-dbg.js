	...
	function(BaseObject, Device, JSONModel, MessageToast, ActionSheet, Dialog, Popover, deletionHelper,
		...
		function getMethods(oTemplateContract) {
			...
			function setEditableNDC(isEditable) {
				// There are several scenarios for chamging from edit to non-edit mode:
				// - Save
				// - Cancel edit
				// - User is navigating away from the edit page in an uncontrolled way (e.g. using browser back)
				// In the last scenario model might still contain some pending changes which should be cleaned up
				if (bIsEditable && !isEditable){
					var oModel = oTemplateContract.oAppComponent.getModel();
					// Only do the cleanup if necessary, since resetChanges has side-effects that should be avoided in the controlled scenario. 
			...
			*/
			...
			// the returned function itself returns a Promise whicvh is resolved as soon as the navigation has been started
		...
		return BaseObject.extend("sap.suite.ui.generic.template.lib.Application", {
			constructor: function(oTemplateContract) {
				Object.assign(this, getMethods(oTemplateContract));
			}
		});
	});
