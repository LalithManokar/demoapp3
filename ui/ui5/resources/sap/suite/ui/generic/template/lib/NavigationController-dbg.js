...
sap.ui.define(["sap/ui/base/Object",
	...
	TemplateComponent, testableHelper, ControlPersonalizationAPI, Log, merge) {
	"use strict";
	...
	function fnCreateComponentInstance(oTemplateContract, oRouteConfig, fnComponentCreateResolve) {
		...
		if (oRouteConfig.component.settings) {
			...
			Object.assign(oSettings, oRouteConfig.component.settings);
		}
	...
	// A promise that resolves to the current intent according to UShell recommendation
		var oMyIntentPromise = (sap.ushell && sap.ushell.Container) ? new Promise(function(fnResolve){
		...
		function getApplicableStateForComponentAddedPromise(sComponentId, sPath, mAppStates){
			...
			return getUrlParameterInfo ? oComponentRegistryEntry.viewRegistered.then(function(){
				...
				return getUrlParameterInfo(sPathNormalized, oCurrentHash.componentsDisplayed[oComponentRegistryEntry.route] === 1).then(function(mNewPars){
					Object.assign(mAppStates, mNewPars);
				});
			}) : Promise.resolve();
		}
		...
		function isIdentityReached(oIdentity){
			...
			var oUnion = Object.assign(Object.create(null), oIdentity.appStates, oCurrentIdentity.appStates);
		...
		function fnSetDisplayMode(iDisplayMode){
			...
			var fnSet = function(bIsAlreadyVisible, oComponent, sComponentId){
				...
				var oRegistryEntry = oTemplateContract.componentRegistry[sComponentId];
				(oRegistryEntry.methods.presetDisplayMode || jQuery.noop)(iDisplayMode, bIsAlreadyVisible);					
			};
		...
		function fnNavigateByExchangingQueryParam(sQueryParam, vValue){
			var oOptions = {
				identity: {
					...
					appStates: Object.assign(Object.create(null), oCurrentIdentity.appStates)
				},
				mode: 1
			};
		...
		// This method navigates to the specified context. Thereby, the entity specified by this context must either eblomg to a root level entity set or its parent must be part of the
		...
		function fnNavigateToSubContext(oContext, bReplace, iDisplayMode, oContextInfo){
			...
			var oTargetIdentity = getTargetIdentityForContext(oContext); // target identity, without app states yet
			if (!oTargetIdentity){
				return;
			}
		...
		function getSpecialDraftCancelPromise(oContext){
			...
			for (var i = oSpecialDraftCancellationInfo.index + 1; i < aPreviousHashes.length; i++){
				...
				if (oTestHash.identity.treeNode.level < oSpecialDraftCancellationInfo.identity.treeNode.level || oSpecialDraftCancellationInfo.identity.keys.some(fnDiffer)){
					return null; // this was a naviagtion step which was not a child of the cancelled draft
				}
			}
			...
			if (oIdentityBefore.treeNode.fCLLevel === 0 || oIdentityBefore.treeNode.fCLLevel === 3){
				Object.assign(oTargetIdentity.appStates, oIdentityBefore.appStates);
		...
		function getSwitchToSiblingPromise(oSiblingContext, iDisplayMode){
			...
			var aSiblingKeysPromises = aTreeNodes.map(function(oTreeNode){ // an array of Promises corresponding to aTreeNodes. The Promises resolve to the keys to be used for that tree node (resp true if no key is needed and undefined if the corresponding column needs to be closed)
				...
				var sContextPath = oTreeNode.noOData ? fnDeterminePathForKeys(oTreeNode, oCurrentIdentity.keys) :
					"/" + oTreeNode.entitySet + "(" + oCurrentIdentity.keys[oTreeNode.level] + ")";
				...
				return oSiblingPromise.then(function(oSiblingContext){
					var oNavTarget = oSiblingContext && routingHelper.determineNavigationPath(oSiblingContext, oTreeNode.navigationProperty);
		...
		// This delivers an identity for navigating to the given context. The result will only contain properties treeNode and keys (but not appState).
		function getTargetIdentityForContext(oContext){
			if (oContext){
				var oTarget = routingHelper.determineNavigationPath(oContext);
				var oTargetNode = oTemplateContract.mEntityTree[oTarget.entitySet];
				// check whether the target node can be reached from the current node
				var oParentNode = oCurrentIdentity.treeNode;
				for (; oParentNode.level > oTargetNode.level - 1;){
					oParentNode = oTemplateContract.mRoutingTree[oParentNode.parentRoute];
				}
				if (oParentNode.sRouteName !== oTargetNode.parentRoute){
					Log.error("Navigation to context from entity set " + oTarget.entitySet + " not possible while being on route " + oCurrentIdentity.treeNode.sRouteName);
					return null;
				}
				var aTargetKeys = oCurrentIdentity.keys.slice(0, oParentNode.level + 1);
				aTargetKeys.push(oTarget.key);				
				return {
		...
		function getAppStatePromiseForUpDownNavigation(oTargetTreeNode, mAppStates, aKeys){
			if (oTargetTreeNode.fCLLevel === 0 || oTargetTreeNode.fCLLevel === 3){
				return getApplicableStateForComponentAddedPromise(oTargetTreeNode.componentId, null, mAppStates);
			}
		...
		function fnNavigateAfterActivation(oActiveContext){
			...
			oTemplateContract.oBusyHelper.setBusy(oAppStatePromise.then(fnNavigateToIdentity.bind(null, oTargetIdentity, true, 1)));
		}
		...
		// This method allows to navigate to a child from the current identity
		// - sChildSpec: the (logical) navigation property which leads from the current identity to the target
		//   More precisely: If the target has been defined with a routingSpec sChildSpec should be the route name defined within this routinfSpec
		//   Otherwise:
		//   * if the current identity is the root sChildSpec should be the target entity set
		//   * if the current identity is not the root sChildSpec should be the navigationProperty from the current identity to the child
		...
		// - bReplace: specified whether the 
		function fnNavigateToChild(sChildSpec, sEmbedded, sKey, bReplace){
			var sRouteName;
			var bWithKey = true;
			for (var i = 0; i < oCurrentIdentity.treeNode.children.length && !sRouteName; i++){
				var sChild = oCurrentIdentity.treeNode.children[i];
				var oChildNode = oTemplateContract.mEntityTree[sChild];
				if (oChildNode[oCurrentIdentity.treeNode.level ? "navigationProperty" : "sRouteName"] === sChildSpec){
					sRouteName = oChildNode.sRouteName;
					bWithKey = !oChildNode.noKey;
				}
			}
			var oEmbeddedComponent = !sRouteName && sEmbedded && oCurrentIdentity.treeNode.embeddedComponents[sEmbedded];
			if (oEmbeddedComponent){
				for (var j = 0; j < oEmbeddedComponent.pages.length && !sRouteName; j++){
					...
					if (oPage.navigationProperty === sChildSpec){
						sRouteName = oCurrentIdentity.treeNode.sRouteName + "/" + sEmbedded + "/" + sChildSpec;
	...
	function constructor(oNavigationController, oTemplateContract){
		...
		Object.assign(oNavigationController, getMethods(oTemplateContract, oNavigationControllerProxy));
		Object.assign(oNavigationControllerProxy, oNavigationController);
		// TODO: this has to be clarified and fixed
		var mRouteToLoadedPromises = Object.create(null);
		oNavigationControllerProxy.oRouter._oViews._getViewWithGlobalId = function(oView) {
			...
			// The component might not be visible in the component registry yet, because the instantiation process is still going on
			var oRet = mRouteToLoadedPromises[oView.viewName];
			if (oRet){
				return oRet;
			}
			...
			oRet = oContainer.loaded();
			mRouteToLoadedPromises[oView.viewName] = oRet;
			return oRet;
		};
