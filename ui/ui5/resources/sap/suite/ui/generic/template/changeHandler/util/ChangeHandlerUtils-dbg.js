sap.ui.define([
	 ...
	 * @public
	 */
	ChangeHandlerUtils.getGroupElements = function(oElement) {
		var oEntityType;
		var sGroupElementQualifier;
		//Control in Object Page
		if (oElement.getId().indexOf(OBJECT_PAGE) > -1) {
			oElement = oElement.getParent();
		    sGroupElementQualifier = AnnotationHelper.getGroupElementQualifier(oElement.getCustomData());
		}
		oEntityType = ChangeHandlerUtils.getODataEntityType(oElement);
		return oEntityType && oEntityType[sGroupElementQualifier];
	};

	/**
	 * Determines the index of a Group Element in the ODataMetaModel.
	 *
	 * @param {sap.m.Column} oTemplData of a group element
	 ...
	 * @public
	 */
	ChangeHandlerUtils.getGroupElementRecordIndex = function(oElement, aGroupElements) {
		if (!oElement || !aGroupElements) {
			return;
		}
		var oTemplData = ChangeHandlerUtils.getTemplatingInfo(oElement);
		var annotationContext = oTemplData.annotationContext;
		var iRecordIndex = -1;
		var aGroupElementsData = aGroupElements['Data'];
		if (annotationContext && aGroupElementsData && aGroupElementsData.length > 0){
			for (var i = 0; i < aGroupElementsData.length; i++){
				...
				if (annotationContext.Action && annotationContext.Action.String  &&  (!aGroupElementsData[i].Action || aGroupElementsData[i].Action.String !== annotationContext.Action.String)){
					continue;
				}
				if (annotationContext.Action && annotationContext.Action.Path  &&  (!aGroupElementsData[i].Action || aGroupElementsData[i].Action.Path !== annotationContext.Action.Path)){
					continue;
				}
				//If SemanticObject present then must match to that present in annotation
				if (annotationContext.SemanticObject && annotationContext.SemanticObject.String  &&  (!aGroupElementsData[i].SemanticObject || (aGroupElementsData[i].SemanticObject.String !== annotationContext.SemanticObject.String))){
					continue;
				}
				if (annotationContext.SemanticObject && annotationContext.SemanticObject.Path  &&  (!aGroupElementsData[i].SemanticObject || (aGroupElementsData[i].SemanticObject.Path !== annotationContext.SemanticObject.Path))){
					continue;
				}
				//If Target present then must match to that present in annotation
				if (annotationContext.Target && annotationContext.Target.AnnotationPath  &&  (!aGroupElementsData[i].Target || (aGroupElementsData[i].Target.AnnotationPath !== annotationContext.Target.AnnotationPath))){
					continue;
				}
				//If Value present then must match to that present in annotation
				if (annotationContext.Value && annotationContext.Value.Path  &&  (!aGroupElementsData[i].Value || (aGroupElementsData[i].Value.Path !== annotationContext.Value.Path))){
					continue;
				}
