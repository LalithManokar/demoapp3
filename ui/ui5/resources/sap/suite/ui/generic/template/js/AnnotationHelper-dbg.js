	...
	var oAnnotationHelper = {
		...
		getNewVariable: function (oContext){
			...
			var	oVariable = {
					...
					// workaround: provide specific setter function since passing formatters inside generic "set" function does not seem to work
					setObjectPageTableType: function(bIsPhone, oSettings, sAnnotationPath, sSapSemantics, sFacetId) {
						if (bIsPhone) {
							oVariable.value = "ResponsiveTable";
						} else {
							sAnnotationPath = oAnnotationHelper.replaceSpecialCharsInId(sAnnotationPath);
							// if an id is defined in a reference facet, sFacetId contains this id, while sAnnotationPath contains the id we would generate if no id is defined in the annotation
							// the id from the annotation should be used to identify the section in the manifest, however for compatibility reasons we also need to read sections identified with the generated id
							var oSectionSettings = oSettings.sections && jQuery.extend(true, {}, oSettings.sections[sFacetId], oSettings.sections[sAnnotationPath]);
							if (oSectionSettings && oSectionSettings.tableSettings && oSectionSettings.tableSettings.type) {
								oVariable.value = oSectionSettings.tableSettings.type;
							} else if (oSettings.tableSettings && oSettings.tableSettings.type) {
								oVariable.value = oSettings.tableSettings.type;
							} else {
								if (sSapSemantics === "aggregate") {
									oVariable.value = "AnalyticalTable";
								} else {
									oVariable.value = "ResponsiveTable";
								}
							}
					...
					setObjectPageTableSettingsProperty: function(sProperty, oSettings, sAnnotationPath, sFacetId) {
						sAnnotationPath = oAnnotationHelper.replaceSpecialCharsInId(sAnnotationPath);
						// if an id is defined in a reference facet, sFacetId contains this id, while sAnnotationPath contains the id we would generate if no id is defined in the annotation
						// the id from the annotation should be used to identify the section in the manifest, however for compatibility reasons we also need to read sections identified with the generated id
						var oSectionSettings = oSettings.sections && jQuery.extend(true, {}, oSettings.sections[sFacetId], oSettings.sections[sAnnotationPath]);
						if (oSectionSettings && oSectionSettings.tableSettings) {
							oVariable.value = oSettings.sections[sAnnotationPath].tableSettings[sProperty];
						} else if (oSettings.tableSettings) {
							oVariable.value = oSettings.tableSettings[sProperty];
						}
					}
				};
		...
		getMultiSelectForTable: function (oFacet, oSections, oSettings) {
			var bGlobalMultiSelect = oSettings && (oSettings.allTableMultiSelect || oSettings.tableSettings && oSettings.tableSettings.multiSelect);
			var oSectionSettings = oSections && oSections[oAnnotationHelper.getStableIdPartFromFacet(oFacet)];
			var bMultiSelectForTable = (oSectionSettings && oSectionSettings.multiSelect);
		...
		getSelectionModeForTable: function (aEntities, sRootEntitySet, oManifestExt, oFacet, oSections, oEntitySet, oRouteConfig, bIsDraftEnabled, oSettings) {
			//Check for selection mode of the table
			var bMultiSelect = oAnnotationHelper.getMultiSelectForTable(oFacet,	oSections, oSettings),
		...
		getSelectionModeResponsiveTable: function (aEntities, sRootEntitySet, oManifestExt, oFacet, oSections, oEntitySet, oRouteConfig, bIsDraftEnabled, oSettings) {
			//Check for selection mode of the table
			var bMultiSelect = oAnnotationHelper.getMultiSelectForTable(oFacet,	oSections, oSettings),
