sap.ui.define([
	...
	"sap/ui/model/odata/AnnotationHelper"
				...
				if (oFocusedControl) {
					...
					switch (oControlType) {
						...
						case "sap.m.HeaderContainerItemContainer" :
							...
							if (oVbox) {
								sRequiredIdSlice = oVbox.getId().replace(/VBox::/g,"");
								oButton = this.getF4ButtonId(sRequiredIdSlice);
							}
							break;
						case "sap.m.CustomListItem" :
							sRequiredIdSlice = oFocusedControl.getId().replace(/CustomListItem::/g,"");
							oButton = this.getF4ButtonId(sRequiredIdSlice);
							break;
						default: break;
					}
					...
					if (oEvent.target.id.length > 0) {
						sRequiredIdSlice = oEvent.target.id.replace(/InteractiveChart::FitlerItem::/g,"").replace(/-interactionArea\S+/g,"");//Bar and Donut
					} else {
						sRequiredIdSlice = oEvent.target.parentElement.id.replace(/InteractiveChart::FitlerItem::/g,"").replace(/-point\S+/g,""); //Lines
					}
					oButton = this.getF4ButtonId(sRequiredIdSlice);
				}
		...
		FilterUtil.getF4ButtonId = function(sId) {
			var sDerivedId =  "Button::" + sId,
			oButton = sap.ui.getCore().byId(sDerivedId);
			return oButton;
		};
