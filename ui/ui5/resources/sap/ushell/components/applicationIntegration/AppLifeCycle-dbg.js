    ...
    function AppLifeCycle () {
        ...
        this.init = function (inAppState, oInViewPortContainer, inSRootIntent, inDisableHomeAppCache, oShellUIServiceChange, aActions, oCacheConfigurations) {
            ...
            if (sap.ushell && sap.ushell.Container) {
                ...
                var bAppStateConfigPersistent = oAppState.getPersistentWhenShared();
                if (bAppStateConfigPersistent === true) {
                    ...
                    URLHelper.triggerEmail = function (sTo, sSubject, sBody, sCc, sBcc) {
                        ...
                        oAppState.makeStatePersistent(sFLPUrl).done(function (sNewURL, sXStateKey, sIStateKey, sXStateKeyNew, sIStateKeyNew) {
        ...
        this.handleOpenStateful = function (bIsInitial, bIsInCache, sToId, oInnerControl, oTarget, sFixedShellHash) {
            ...
            if (Storage.get(sToId) && bIsInitial === false) {
                oPromise = this.reuseStateFulContainerAndRestore(oInnerControl, sToId, oHandler, sFixedShellHash);
            } else {
                oPromise = this.reuseStateFulContainer(oInnerControl, oTarget.url, sToId, oHandler, sFixedShellHash);
        ...
        this.open = function (sIntent, sAppId, oShellHash, oTarget, fnWrapper, sFixedShellHash, oMetadata) {
            ...
            oDeferredDestroyApp.then(function () {
                ...
                if (bReuseStatefulContainer) {
                    oPromiseAsyncAppCreate = that.handleOpenStateful(bIsInitial, bIsInCache, "application" + sAppId, oInnerControl, oTarget, sFixedShellHash);
                } else if (bReuseAnExistingAppSession) {
                    oPromiseAsyncAppCreate = that.reuseApplicationContainer(oInnerControl, oTarget.applicationType, oTarget.url);
                }
        ...
        this.handleControl = function (sIntent, sAppId, oShellHash, oTarget, fnWrapper, sFixedShellHash) {
            ...
            return new Promise(function (fnResolve, fnReject) {
                that.leave(oCurrentPage, sCurrentPageId).then(function() {
                    that.open(sIntent, sAppId, oShellHash, oTarget, fnWrapper, sFixedShellHash, oMetadata).then(fnResolve, fnReject);
                }, function (vError) {
                    Log.error(vError && vError.message || vError);
                    fnReject(vError && vError.message || vError);
                });
            });
        };
