...
sap.ui.define([
    ...
    function fnValidateAddingHeadEndItems (aExistingIds, aIdsToAdd) {
        ...
        // If some of the buttons are already added, do not try to add them again but continue adding others
        for (index = 0; index < aExistingIds.length; index++) {
            var iIndex = aIdsToAdd.indexOf(aExistingIds[index]);
            if (iIndex > -1) {
                aIdsToAdd.splice(iIndex, 1); // remove the entry
