...
sap.ui.define([
    ...
    var URL_PARAMS_FILTER = ["sap-ach", "sap-target-navmode", "sap-fiori-id", "sap-hide-intent-link", "sap-priority", "sap-tag",
    ...
    */
    ...
    function isTrustedPostMessageSource (oContainer, oMessage) {
        ...
        if (new UriParameters(window.location.href).get("sap-post") === "true" && oDomRef && oDomRef.getAttribute && oDomRef.getAttribute("sap-iframe-app") == "true") {
    ...
    function getIFrame () {
        ...
        if (!oIFrame || oIFrame.tagName !== "IFRAME") {
            if (new UriParameters(window.location.href).get("sap-post") === "true" && oIFrame && oIFrame.getAttribute && oIFrame.getAttribute("sap-iframe-app") == "true") {
    ...
    function render (oRenderManager, oContainer, sApplicationType, sUrl, sAdditionalInformation) {
        ...
        if (new UriParameters(window.location.href).get("sap-post") === "true" && utils.isApplicationTypeEmbeddedInIframe(sApplicationType)) {
    ...
    ApplicationContainer.prototype.setNewTRApplicationContext = function (sUrl, sApplicationType) {
        ...
        if (utils.isApplicationTypeEmbeddedInIframe(sApplicationType)) {
            var sTargetNavigationMode = this.getTargetNavigationMode();

            // amend already resolved url with additional parameters
            sUrl = ApplicationContainer.prototype._adjustNwbcUrl(sUrl, sApplicationType, sTargetNavigationMode, this.getIsStateful());

            // add this container to list of NWBC-containing containers
            utils.localStorageSetItem(this.globalDirtyStorageKey,
                sap.ushell.Container.DirtyState.INITIAL);
        }
    ...
    ApplicationContainer.prototype.setNewApplicationContext = function (sApplicationType, sUrl) {
        ...
        return this.postMessageToIframe(oRequestMessage, oIFrame, true/* bWaitForResponse */)
            .then(
                function () {
                    return fnSetNewContext.call(that, sUrl, sApplicationType);
                },
                function (oEventData) {
                    return Promise.reject({
                        eventData: oEventData,
