    ...
    sap.ui.controller("sap.ushell.renderers.fiori2.Shell", {
        ...
        onInit: function () {
            ...
            sap.ushell.Container.getServiceAsync("Menu").then(function (oMenuService) {
                return oMenuService.isMenuEnabled().then(function (bIsEnabled) {
                    if (bIsEnabled) {
                        var oComponentContainer = sap.ui.getCore().byId("subMenuComponentContainer");
                        Component.create({
                            name: "sap.ushell.components.shell.SubMenu"
                        }).then(function (oSubMenuComponent) {
                            oComponentContainer.setComponent(oSubMenuComponent);
                        });
                    }
                });
            });
        ...
        _openAppInNewWindowAndRestore: function (oResolvedHashFragment) {
            ...
            if (utils.isNativeWebGuiNavigation(oResolvedHashFragment)) {
                try {
                    ...
                    if (this.oShellNavigation.isInitialNavigation()) {
                        this._resumeAppRouterIgnoringCurrentHash();
                        this._windowHistoryBack(1);
                        return;
                    }
            ...
            setTimeout(function () {
                setTimeout(function () {
                    ...
                    if (Config.last("/core/shell/model/currentState/stateName") === "app") {
                        var oAppTitleDomRef = sap.ui.getCore().byId("shellAppTitle").getFocusDomRef();
                        arg = oAppTitleDomRef && oAppTitleDomRef.getAttribute("id");
                    }
        ...
        _openAppNewWindow: function (sUrl) {
            ...
            if (sUrl.trim().indexOf("sap-nwbc://") === 0 && (Device.browser.edge || Device.browser.msie)) {
                ...
                if (!oNewWin && !(Device.browser.msie || Device.browser.edge)) {
        ...
        onBeforeNavigate: function (oEvent) {
            var oSubMenuComponentContainer = sap.ui.getCore().byId("subMenuComponentContainer");

            if (oSubMenuComponentContainer.getComponent()) {
                var oShellHomeComponentContainer = this.getOwnerComponent().byId("Shell-home-component-container");
                if (oEvent.getParameter("to") === oShellHomeComponentContainer) {
                    oSubMenuComponentContainer.setVisible(true);
                } else {
                    oSubMenuComponentContainer.setVisible(false);
                }
                oSubMenuComponentContainer.rerender();
            }
        },
        ...
        logOpenAppAction: function (oResolvedHashFragment, sAppPart) {
            ...
            sap.ushell.Container.getServiceAsync("URLParsing").then(function (oURLParsing) {
                ...
                if (Config.last("/core/shell/model/enableTrackingActivity") && sFixedShellHash.indexOf("#Action-search") === -1) {
                    setTimeout(function () {
