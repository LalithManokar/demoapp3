sap.ui.define([
	...
	TotaraLoader.prototype.request = function(sceneVeId, contextParams, authorizationHandler) {
		...
		if (contextParams.activateView) {
			viewOptions.id = contextParams.activateView;
		...
		commandInStr = TotaraUtils.createCommand(Command.getView, viewOptions);
		/*
		{
			var options = {
				pushMaterials: true,
			...
			commandInStr = TotaraUtils.createCommand(Command.getTree, options);
		}
		*/
	...
	TotaraLoader.prototype.processCommand = function(name, jsonContent, binaryContent) {
		...
		switch (name) {
			...
			case Command.notifyError: result = { error: jsonContent }; break;
