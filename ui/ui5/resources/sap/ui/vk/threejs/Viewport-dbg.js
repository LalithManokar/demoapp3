	...
	AnimationTrackType
		...
		Viewport.prototype.handleKeyFrameAdded = function(event) {
			...
			if (data.trackType === AnimationTrackType.rotate) {
				data.targetNodeRef.quaternion.set(data.keyFrameValue[ 0 ], data.keyFrameValue[ 1 ], data.keyFrameValue[ 2 ], data.keyFrameValue[ 3 ]);
			} else if (data.trackType === AnimationTrackType.translate) {
				data.targetNodeRef.position.set(data.keyFrameValue[ 0 ], data.keyFrameValue[ 1 ], data.keyFrameValue[ 2 ]);
			} else if (data.trackType === AnimationTrackType.scale) {
				data.targetNodeRef.scale.set(data.keyFrameValue[ 0 ], data.keyFrameValue[ 1 ], data.keyFrameValue[ 2 ]);
			} else if (data.trackType === AnimationTrackType.opacity) {
				data.targetNodeRef.traverse(function(node) {
					if (node.material && node.material.opacity) {
						node.userData.opacity = data.keyFrameValue[ 0 ];
				...
				data.targetNodeRef.traverse(function(node) {
					if (node.material && node.material.color) {
						node.material.color.setRGB(data.keyFrameValue[ 0 ], data.keyFrameValue[ 1 ], data.keyFrameValue[ 2 ]);
					}
				});
			}
			this.setShouldRenderFrame();
		...
		Viewport.prototype.handleKeyFrameDeleted = function(event) {
			...
			if (keyFrame) {
				if (data.trackType === AnimationTrackType.rotate) {
					...
					if (value) {
						data.targetNodeRef.quaternion.set(value[ 0 ], value[ 1 ], value[ 2 ], value[ 3 ]);
					}
				} else if (data.trackType === AnimationTrackType.translate) {
					data.targetNodeRef.position.set(keyFrame.value[ 0 ], keyFrame.value[ 1 ], keyFrame.value[ 2 ]);
				} else if (data.trackType === AnimationTrackType.scale) {
					data.targetNodeRef.scale.set(keyFrame.value[ 0 ], keyFrame.value[ 1 ], keyFrame.value[ 2 ]);
				} else if (data.trackType === AnimationTrackType.opacity) {
					data.targetNodeRef.traverse(function(node) {
						if (node.material && node.material.opacity) {
							node.userData.opacity = keyFrame.value[ 0 ];
							node._vkUpdateMaterialOpacity();
						}
					});
				} else if (data.trackType === AnimationTrackType.color) {
					data.targetNodeRef.traverse(function(node) {
						if (node.material && node.material.color) {
							node.material.color.setRGB(keyFrame.value[ 0 ], keyFrame.value[ 1 ], keyFrame.value[ 2 ]);
						}
					});
				}
			} else {
		...
		*/
		Viewport.prototype.hitTest = function(x, y) {
			...
			if (intersects) {
				for (var i in intersects) {
					...
					while (object.parent && ((object.userData.treeNode && object.userData.treeNode.skipIt) || (!object.name && object.children.length === 0))) {
		...
		Viewport.prototype._setContent = function(content) {
			...
			if (scene) {
				...
				if (initialView) {
					...
					this._resetNodesMaterialAndOpacityByCurrentView();
			...
			if (nodeValues) {
				...
				while (!nextValue.done) {
					...
					if (data.opacity !== undefined) {
						node.userData.opacity = data.opacity;
						node._vkUpdateMaterialOpacity();
					}
		...
		Viewport.prototype._runAnimation = function(dragging) {
			...
			var currentPlayback, animationSequence, pi;
			for (pi = 0; pi < playbacks.length; pi++) {
				currentPlayback = playbacks[pi];
				animationSequence = this._scene.getAnimationSequence(currentPlayback.getSequenceId());
				if (animationSequence) {
					if (!animationSequence.userData) {
						animationSequence.userData = {};
					}
					animationSequence.userData.playbackCount = 0;
				}
			}
			for (pi = 0; pi < playbacks.length; pi++) {
				currentPlayback = playbacks[pi];
				...
				var animationClip;
				if (animationSequence) {
					if (animationSequence.userData.playbackCount === 0) {
						animationClip = animationSequence.getNativeAnimationClip();
						animationSequence.userData.playbackCount = 1;
					} else {
						var newId = currentPlayback.getSequenceId() + animationSequence.userData.playbackCount.toString();
						var clonedAnimationSequence = this._scene.getAnimationSequence(newId);
						if (!clonedAnimationSequence) {
							clonedAnimationSequence = this._scene.createAnimationSequence(newId);
							clonedAnimationSequence.clone(animationSequence, true);
						}

						animationClip = clonedAnimationSequence.getNativeAnimationClip();
						animationSequence.userData.playbackCount++;
					}
					this.AddHandlersToAnimationSequenceEvents(animationSequence);
				}
				...
				if (animationClip) {
					...
					var repeat = currentPlayback.getRepeat();
					if (!repeat) {
						repeat = 1;
					}
					...
					if (currentPlayback.getInfinite()) {
						...
						if (!currentPlayback.getReversed()) {
							clipAction.setLoop(THREE.LoopRepeat, repeat);
								...
								clipAction.setLoop(THREE.LoopRepeat, repeat - 1);
						} else {
		...
		*/
		...
		Viewport.prototype._activateSingleView = function(view) {
			...
			if (camera) {
				...
				this._viewportGestureHandler.activateCamera(camera.getCameraRef(), timeInterval, this._playingProcedure);
			} else {
		...
		Viewport.prototype._updateMaterialInNode = function(nodeInfo) {
			...
			for (i = 0; i < node.children.length; i++) {
				...
				if (child.material && (child.userData.animatedOpacity || child.userData.animatedColor)) {
			...
			if (node.userData.materialId !== undefined) {
				updateMaterial = nodeInfo.materialId !== node.userData.materialId;
			} else {
				updateMaterial = nodeInfo.materialId !== undefined;
			}

			var updateOpacity;
			if (node.userData.opacity !== undefined) {
				updateOpacity = nodeInfo.opacity !== node.userData.opacity;
			} else {
				updateOpacity = nodeInfo.opacity !== undefined || node.userData.animatedOpacity;
			}

			node.userData.materialId = nodeInfo.materialId;
			node.userData.opacity = nodeInfo.opacity;

			if (updateMaterial || updateOpacity) {
				var nodeMaterial = this._scene.getMaterial(node.userData.materialId); // sap.ui.vk.Material || undefined

				for (i = 0; i < node.children.length; i++) {
					child = node.children[ i ];
					if (child.material) {
						if (updateMaterial) {
							var newMaterial = nodeMaterial ? nodeMaterial : this._scene.getMaterial(child.userData.initialMaterialId); // sap.ui.vk.Material || undefined
							if (newMaterial) {
								var materialRef = newMaterial.getMaterialRef();
								child.userData.materialId = materialRef.userData.materialId;
								child.userData.originalMaterial = materialRef;
								child.material = materialRef.clone();
								if (this._scene._sceneBuilder) {
									this._scene._sceneBuilder._attachMaterialClone(child.userData.materialId, child.material);
								}
								if (child.userData.animatedOpacity) {
									child.material.transparent = true;
								}
		...
		Viewport.prototype._resetNodesMaterialAndOpacityByCurrentView = function() {
			...
			if (nodeInfo) {  // for totaraLoader
				nodeInfo.forEach(this._updateMaterialInNode.bind(this));
			}
		};
