...
sap.ui.define([
			...
			var ThreeJsCameraController = function(vp) {
				...
				this.zoom = function(zoomFactorScaler) {
					...
					if (!camera.userData.isRedlineActivated) {
						...
						viewport.getScene().getSceneRef()._expandBoundingBox(boundingBox, true, true);
				...
				this.startAnimatingCameraUpdate = function(timeInterval, enableFlyToTime) {
					var camera = viewport.getCamera().getCameraRef();
					...
					if (!enableFlyToTime){ // If we are playing a procedure we want camera to update regardless, but if we are playing a single view we do not
	...
	ViewportGestureHandler.prototype.activateCamera = function(camera, timeInterval, enableFlyToTime) {
		...
		this._cameraController.startAnimatingCameraUpdate(timeInterval, enableFlyToTime);
		return this;
	};
