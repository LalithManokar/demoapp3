...
sap.ui.define([
	 ...
	 * @class A metadata-driven table to simplify the usage of existing tables, such as the <code>sap.m.Table</code> and <code>sap.ui.table</code> controls.
	 *        <h3>Overview</h3>
	 *        The <code>Table</code> control creates a <code>ResponsiveTable</code> or <code>GridTable</code> based on the configuration specified.
	 *        <h3>Structure</h3>
	 *        The <code>columns</code> aggregation must be specified with the columns you require, along with with the template for the cell. The cell
	 *        template can also be created lazily via the {@link #getMetadataDelegate table delegate}.<br>
	 *        <b>Note:</b>
	 *        The control is experimental and the API/behavior is not finalized and hence this should not be used for productive usage.
	...
	var Table = Control.extend("sap.ui.mdc.Table", {
		...
		metadata: {
			...
			properties: {
				...
				/**
				 * Contains the collection, model, and other relevant information enabling the delegate to function properly.<br>
				 * The <code>metadataInfo</code> is processed and used by the delegates and not by the control itself.<br>
				 * <b>Note:</b>Do not bind or modify the property. Once the <code>metadataInfo</code> is used, this property is not used anymore.
				 *
				 * @experimental
				 */
				metadataInfo: {
				...
				/**
				 * Contains the information required for binding the rows/items of the table. Changes to this property are only reflected once
				 * the next complete rebind on the table occurs.<br>
				...
				/**
				 * Binds the table automatically after the initial creation or re-creation of the table using the relevant
				 * <code>metadataInfo</code> and <code>rowBindingInfo</code>.
				 */
				autoBindOnInit: {
					type: "boolean",
					group: "Misc",
				...
				/**
				 * Determines whether the header text is shown in the table. Even when set to <code>false</code>, the given header text is used to label
				 ...
				 * <b>Note:</b><br>
				 * To improve the performance of your application, you do not want to send dedicated OData requests. Therefore, you must configure
				 * the count mode either in the model or in the binding of the table.<br>
				 ...
				 * <code>ResponsiveTable</code>, then this property refers to the {@link sap.m.ListBase#getGrowingThreshold growingThreshold} property
				 * of <code>sap.m.Table</code> If the <code>type</code> property is set to the <code>Table</code>, then this property refers to the
				 * {@link sap.ui.table.Table#getThreshold threshold} property of <code>sap.ui.table.Table</code><br>
			...
			associations: {
				/**
				 * Control or object which enables the table to do filtering, such as {@link sap.ui.mdc.FilterBar}. Also see {@link sap.ui.mdc.IFilter}.
				 */
				filter: {
...
// GridTable
	...
	Table.prototype._getFilters = function() {
		var oFilter = sap.ui.getCore().byId(this.getFilter());
		return oFilter ? oFilter.getFilters() : [];
	};
	...
	Table.prototype.bindRows = function(oBindingInfo) {
		...
		if (this._oTable && oBindingInfo.path) {
			...
			// Update filters
			if (this.getFilter()) {
				this._oBindingInfo.filters = this._getFilters();
			}
