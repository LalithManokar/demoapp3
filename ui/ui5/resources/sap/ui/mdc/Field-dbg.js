...
sap.ui.define([
	...
	'sap/ui/mdc/base/FieldBaseRenderer',
	...
	Field.prototype._handleModelContextChange = function(oEvent) {
		...
		if (this.getBinding("additionalValue")) {
			this.getBinding("additionalValue").checkUpdate(true);
		}
	};
	...
	Field.prototype._observeChanges = function(oChanges) {
		...
		if (oChanges.name === "value") {
			this._vValue = _adjustValue.call(this, oChanges.current, oChanges.old);
	...
	function _compareValues(vValue1, vValue2, bUpdateCheck) {
		...
		if (!bEqual
				&& (sDataType === "sap.ui.model.odata.type.Unit" || sDataType === "sap.ui.model.odata.type.Currency"
					|| sDataType === "sap.ui.model.type.Unit" || sDataType === "sap.ui.model.type.Currency")
	...
	function _initializeType(vValue) {
		...
		if (!this._bTypeInitialized) {
			...
			if (oDataType && (oDataType.isA("sap.ui.model.odata.type.Unit") || oDataType.isA("sap.ui.model.odata.type.Currency"))
				...
				oDataType.formatValue(vValue, "string");
				this._bTypeInitialized = true;
				this._mCustomUnits = vValue[2]; // TODO: find a better way to provide custom units to internal type
			}
		}
