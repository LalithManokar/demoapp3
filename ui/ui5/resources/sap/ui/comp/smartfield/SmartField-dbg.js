...
sap.ui.define([
	...
	SmartField.prototype.init = function() {
		...
		this.attachEvent("innerControlsCreated", this._setOnInnerControl);
	};
	...
	SmartField.prototype.setEditableProperty = function(sPropertyName, bEditable, bSuppressInvalidate) {
		...
		// On change from editable -> display -> we need to re-bind
		if (bEditableStateChanged && this._oFactory && !bEditable && bOldEditable) {
	...
	SmartField.prototype._checkErrors = function(oControl, mSettings) {
		...
		function handleSuccess() {
			...
			if (oBinding.hasValidation()) {
				oControl.fireValidationSuccess(mSuccessParameters, false, true); // bAllowPreventDefault, bEnableEventBubbling
			}
		}
