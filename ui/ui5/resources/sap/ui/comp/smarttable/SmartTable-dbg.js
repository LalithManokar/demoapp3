...
sap.ui.define([
	...
	SmartTable.prototype._createInfoToolbar = function() {
		...
		if (this._isMobileTable) {
			...
			if (iFiltersLength) {
				var aFiltersColumnLabel = [], oResourceBundle = sap.ui.getCore().getLibraryResourceBundle("sap.ui.comp"), sFilterText = oResourceBundle.getText("SMARTTABLE_FILTERED_BY") + " ";
				aFilters.forEach(function(oFilter) {
					var sFilterColumnLabel = this._getColumnLabel(oFilter.columnKey);
					if (aFiltersColumnLabel.indexOf(sFilterColumnLabel) === -1) {
			...
			this._oCurrentVariant.filter.filterItems.forEach(function(oModelItem) {
				...
				if (sPath) {
					...
					if (oModelItem.operation === "Empty") {
						...
						aFilterArrReference.push(new Filter(sPath, oModelItem.exclude ? FilterOperator.NE : oModelItem.operation, oValue1, oValue2));
					}
