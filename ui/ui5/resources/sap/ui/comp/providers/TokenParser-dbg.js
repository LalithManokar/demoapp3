...
sap.ui.define([
	...
	function(library, ManagedObject, Token, mLibrary, FormatUtil) {
		...
		var TokenParser = function(sDefaultOperation) {
			...
			this._mTypeOperations = {
				"default": [
					...
					// P13nConditionOperation.NotEQ//"NE"
					"NE"
				],
				"string": [
					P13nConditionOperation.Empty,
					// P13nConditionOperation.NotEmpty,//"ExcludeEmpty",
					"ExcludeEmpty",
		...
		TokenParser.rangeOperations["ExcludeEmpty"] = {
			regex: new RegExp(
				"^" +
				FormatUtil.getFormattedRangeText(
					ValueHelpRangeOperation.Empty,
		...
		TokenParser.rangeOperations["NE"] = {
			regex: /^![(]?=(.+?)[)]?$/,
			operator: "NE",
			example: "!=foo",
			template: "!(=$0)",
		...
		TokenParser.excludeOperationsMapping["NE"] = P13nConditionOperation.EQ;
		TokenParser.excludeOperationsMapping["ExcludeEmpty"] = P13nConditionOperation.Empty;
		/**
		...
		TokenParser.prototype._init = function() {
			...
			// Exclude operations
			this.createOperation(TokenParser.rangeOperations["NE"]);
			this.createOperation(TokenParser.rangeOperations["ExcludeEmpty"]);
		};
		...
		TokenParser.prototype.associateInput = function(oInput) {
			...
			this._aOrgValidators = this._oInput && this._oInput.isA('sap.m.MultiInput') ? this._oInput._tokenizer._aTokenValidators.slice() : [];
		...
		TokenParser._matchOperationFromText = function(sText){
			...
			if (aMatchingOperations.length > 1 ) {
				...
				if (sText === FormatUtil.getFormattedRangeText(ValueHelpRangeOperation.Empty, null, null, true) &&
					aMatchingOperations.indexOf(TokenParser.rangeOperations["ExcludeEmpty"].operator) > -1 ){
					aMatchingOperations = [TokenParser.rangeOperations["ExcludeEmpty"].operator];
				}
