		...
		var Extension = ControllerExtension.extend("sap.fe.controllerextensions.Transaction", {
			...
			deleteDocument: function(vContexts, mParameters, oLocalUIModel) {
						...
						if (mParameters.numberOfSelectedContexts === 1 && mParameters.numberOfSelectedContexts === vContexts.length) {
							...
							oDeleteMessage.text =
								aDeletableContexts.length === 1
									? localI18nRef.getText("OBJECT_PAGE_CONFIRM_DELETE_WITH_OBJECTTITLE_SINGULAR")
									: localI18nRef.getText("OBJECT_PAGE_CONFIRM_DELETE_WITH_OBJECTTITLE_PLURAL");
						} else {
							...
							oDeleteMessage.text =
								aDeletableContexts.length === 1
									? localI18nRef.getText("OBJECT_PAGE_CONFIRM_DELETE_WITH_OBJECTTITLE_PLURAL")
									: localI18nRef.getText("OBJECT_PAGE_CONFIRM_DELETE_WITH_OBJECTTITLE_SINGULAR");
							oDeleteMessage.nonDeletableText = localI18nRef.getText(
						...
						if (
							...
							if (
								...
								if (mParameters.unSavedContexts.length === 1) {
									oDeleteMessage.text = localI18nRef.getText(
								...
								if (mParameters.unSavedContexts.length === 1) {
									oDeleteMessage.checkBoxText = localI18nRef.getText(
				...
				var sTitle = mParameters.numberOfSelectedContexts
					? localI18nRef.getText("OBJECT_PAGE_DELETE_DIALOG", [mParameters.numberOfSelectedContexts])
					: localI18nRef.getText("OBJECT_PAGE_DELETE");
								...
								aContexts.map(function(oContext) {
									oContext
										...
										.then(function() {
											...
											aContexts.length === 1
												? MessageToast.show(localI18nRef.getText("OBJECT_PAGE_DELETE_TOAST_SINGULAR"))
												: MessageToast.show(localI18nRef.getText("OBJECT_PAGE_DELETE_TOAST_PLURAL"));
