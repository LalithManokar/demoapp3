...
sap.ui.define(
	...
	function(
		...
		var Extension = ControllerExtension.extend("sap.fe.controllerextensions.EditFlow", {
			 ...
			 *
			 * @function
			 * @name sap.fe.controllerextensions.EditFlow#syncTask
				...
				return this.syncTask().then(function() {
					return new Promise(function(resolve, reject) {
						...
						if (typeof vListBinding === "object") {
							...
							oGetListBinding = that.base
								.getView()
								.getModel()
								.getBindingForReference(vListBinding);
						}
								...
								switch (sCreationMode) {
									...
									case "Sync":
										...
										oCreation.then(function(oNewDocumentContext) {
											that.base.routing.navigateToContext(oNewDocumentContext, mArgs);
										});
										break;
									case "Inline":
			...
			formatDraftOwnerText: function(
				...
				if (sFlag) {
					sDraftOwnerDescription += sDraftInProcessByUser
						? oResourceBundle.getText("DRAFTINFO_GENERIC_LOCKED_OBJECT_POPOVER_TEXT") + " "
						: oResourceBundle.getText("DRAFTINFO_LAST_CHANGE_USER_TEXT") + " ";
				}
				sDraftOwnerDescription += sUserDescription
					? oResourceBundle.getText("DRAFTINFO_OWNER", [sUserDescription])
					: oResourceBundle.getText("DRAFTINFO_ANOTHER_USER");
				...
				return that.base.transaction.getProgrammingModel(oBinding).then(function(sProgrammingModel) {
					...
					oBinding.attachEvent("patchSent", function() {
						...
						that.base.routing.setDirtyState(
							//contextBinding
							(oBinding.getBoundContext && oBinding.getBoundContext()) || "", //TODO: listBinding (of tables on object pages) will return undefined. What is correct?
							true
						); //Pass the context
