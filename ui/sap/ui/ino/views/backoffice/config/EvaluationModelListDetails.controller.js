 ...
 * @copyright@
 */
sap.ui.controller("sap.ui.ino.views.backoffice.config.EvaluationModelListDetails", {
    ...
    getBoundObject : function(oBinding, absolute, oType){
        ...
        if(oType){
            if (absolute){
                return { path :  this.getModelPrefix() + "/" + oBinding, type: oType };
            }else{
                return { path : this.getModelPrefix() + oBinding, type: oType };
            }
         }else{
             if (absolute){
                 return { path : this.getModelPrefix() + "/" + oBinding };
             }else{
                 return { path : this.getModelPrefix() + oBinding };
             }
        }
